...
public class VABlocks {
    ...
    static {
        ...
        RAW_STEEL_BLOCK = register("raw_steel_block", new Block(FabricBlockSettings.create().mapColor(MapColor.GRAY).requiresTool().strength(5.0F, 300.0F)));
        STEEL_BLOCK = register("steel_block", new Block(FabricBlockSettings.create().mapColor(MapColor.GRAY).sounds(BlockSoundGroup.NETHERITE).requiresTool().hardness(5.0F).resistance(300.0F)));
        ...
        SPOTLIGHT_LIGHT = register("spotlight_light", new SpotlightLightBlock(FabricBlockSettings.create().strength(-1.0F, 3600000.8F).sounds(BlockSoundGroup.INTENTIONALLY_EMPTY).replaceable().dropsNothing().nonOpaque().luminance((state) -> state.get(SpotlightLightBlock.LIT) ? 13 : 0).requires(VirtualAdditions.PREVIEW)));
        ...
        GREENCAP_MUSHROOM = register("greencap_mushroom", new GreencapMushroomBlock(AbstractBlock.Settings.create().mapColor(MapColor.DARK_GREEN).noCollision().breakInstantly().sounds(BlockSoundGroup.NETHER_SPROUTS).offset(AbstractBlock.OffsetType.XZ).luminance((state) -> 2).pistonBehavior(PistonBehavior.DESTROY)));
        TALL_GREENCAP_MUSHROOMS = register("tall_greencap_mushrooms", new TallGreencapMushroomBlock(AbstractBlock.Settings.create().mapColor(MapColor.DARK_GREEN).noCollision().breakInstantly().sounds(BlockSoundGroup.NETHER_SPROUTS).offset(AbstractBlock.OffsetType.XZ).luminance((state) -> 5).pistonBehavior(PistonBehavior.DESTROY)));
        WEBBED_SILK = register("webbed_silk", new SilkFluffBlock(AbstractBlock.Settings.create().strength(0.25F).sounds(BlockSoundGroup.POWDER_SNOW).mapColor(MapColor.DARK_AQUA).nonOpaque().solidBlock((state, world, pos) -> false).suffocates(((state, world, pos) -> false)).blockVision((state, world, pos) -> false).burnable().allowsSpawning((state, world, pos, type) -> type == VAEntityType.LUMWASP)));
